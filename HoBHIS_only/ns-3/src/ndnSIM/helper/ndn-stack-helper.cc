...
#include "../model/ndn-hobhis-net-device-face.h"
...
void
StackHelper::EnableHobhis (bool enable,
						   bool client_server,
                           uint32_t maxInterest,
                           uint32_t queueTarget,
                           double design,
                           bool dynamic_design)
{
	  NS_LOG_INFO ("EnableHobhis: " << enable);
	  m_hobhisEnabled = enable;
	  m_client_server = client_server;
	  m_maxInterest = maxInterest;
	  m_design = design;
	  m_queueTarget = queueTarget;
	  m_dynamic_design = dynamic_design;
}
  ...
  for (uint32_t index=0; index < node->GetNDevices (); index++)
    {
      ...
      if (m_hobhisEnabled)
      {
    	  face = CreateObject<HobhisNetDeviceFace> (node, device);
    	  face->SetAttribute("MaxInterest", UintegerValue (m_maxInterest));
    	  face->SetAttribute("Design", DoubleValue(m_design));
    	  face->SetAttribute("QueueTarget", UintegerValue(m_queueTarget));
    	  face->SetAttribute("HoBHISEnabled", BooleanValue(m_hobhisEnabled));
    	  face->SetAttribute("ClientServer", BooleanValue(m_client_server));
    	  face->SetAttribute("DynamicDesign", BooleanValue(m_dynamic_design));
    	  DataRateValue dataRate;
    	  device->GetAttribute ("DataRate", dataRate);

    	  uint64_t dRate = dataRate.Get().GetBitRate();
    	  face->SetCapacity(dRate);
    	  std::cout<<face<<" Data Rate stack "<<dRate<<"\t"<<m_client_server<<std::endl;
      }
      else if (!m_hobhisEnabled)
      {
    	  ...
    	  uint64_t dRate = dataRate.Get().GetBitRate();
    	  face->SetCapacity(dRate);
    	  std::cout<<face<<" Data Rate "<<dRate<<"\t"<<m_client_server<<std::endl;
      }
